...
# ── SVG drone icon loading (cached at module level) ──────────────────────
_DRONE_SVG_VERTICES: np.ndarray | None = None


def _load_drone_svg_vertices() -> np.ndarray:
    """Parse the quadcopter SVG and return normalized 2-D vertices.

    The returned array has shape (N, 2), centred at (0, 0), with
    max(abs) ≈ 1.  Uses ``svg.path`` (pure Python, no system deps).
    """
    global _DRONE_SVG_VERTICES
    if _DRONE_SVG_VERTICES is not None:
        return _DRONE_SVG_VERTICES
    ...
    svg_path = (
        ...
        / "figures"
        / "drone-quadcopter-top-svgrepo-com.svg"
    )
    ...
    for path_el in root.findall(".//{http://www.w3.org/2000/svg}path"):
        ...
        if total < 1e-6:
            continue
        for seg in parsed:
            n = max(2, int(seg.length() / total * 400))
            for t in np.linspace(0.0, 1.0, n, endpoint=True):
                pt = seg.point(t)
                all_pts.append([pt.real, pt.imag])
    v = np.array(all_pts, dtype=float)
    centre = v.mean(axis=0)
    v -= centre
    scale = float(np.max(np.abs(v)))
    if scale > 1e-12:
        v /= scale
    _DRONE_SVG_VERTICES = v
    return v


def _drone_outline(center_xy: np.ndarray, size: float = 0.03, yaw: float = 0.0) -> np.ndarray:
    """2-D drone outline (N×2) centred at *center_xy* with bounding half-size *size*."""
    verts = _load_drone_svg_vertices().copy()  # (N, 2), centred, max(abs)≈1
    if abs(yaw) > 1e-12:
        c, s = math.cos(yaw), math.sin(yaw)
        R = np.array([[c, -s], [s, c]], dtype=float)
        verts = verts @ R.T
    verts = verts * size + np.asarray(center_xy, dtype=float).reshape(1, 2)
    return verts


def _drone_outline_3d(center_xyz: np.ndarray, size: float = 0.03, yaw: float = 0.0) -> np.ndarray:
    """3-D drone outline (N×3) lying in the xy-plane at the given z."""
    xy = _drone_outline(center_xyz[:2], size=size, yaw=yaw)
    z = float(np.asarray(center_xyz, dtype=float).reshape(3)[2])
...
class UndirectedFormationController:
    ...
    def tension_law_output(self, agents: list[DoubleIntegratorAgent]) -> np.ndarray:
        rope_force = np.zeros((4, 3), dtype=float)
        p_stack = np.vstack([agent.pos for agent in agents])
        v_stack = np.vstack([agent.vel for agent in agents])
...
def simulate(config: SimulationConfig) -> tuple[list[DoubleIntegratorAgent], UndirectedFormationController, np.ndarray]:
    agents = [
        DoubleIntegratorAgent(1, [0.0, 0.34, 0.0]),
        DoubleIntegratorAgent(2, [0.0, 0.0, 0.0]),
        DoubleIntegratorAgent(3, [0.34, 0.0, 0.0]),
        DoubleIntegratorAgent(4, [0.34, 0.34, 0.0]),
    ]
    ...
    for step in range(sim_steps):
        rope_force = controller.tension_law_output(agents)
        p_stack = np.vstack([agent.pos for agent in agents])
        for idx, agent in enumerate(agents):
            disturbance = get_disturbance(t, idx, p_stack, config)
            agent.step(
                config.sim_dt,
...
def _plot_xy_trajectory(agents: list[DoubleIntegratorAgent], output_dir: Path) -> None:
    ...
    for agent in agents:
        ...
        drone = _drone_outline(pos_hist[-1, :2], size=UAV_SQUARE_HALF_SIZE)
        ax.plot(drone[:, 0], drone[:, 1], color="black", linewidth=0.8)
        ax.text(
...
def _plot_3d_trajectory(agents: list[DoubleIntegratorAgent], output_dir: Path) -> None:
    ...
    for agent in agents:
        ...
        drone = _drone_outline_3d(pos_hist[-1], size=UAV_SQUARE_HALF_SIZE)
        ax.plot(drone[:, 0], drone[:, 1], drone[:, 2], color="black", linewidth=0.8)
...
def _save_animation(
    ...
    def update(frame_idx: int):
        ...
        for idx in range(len(agents)):
            ...
            drone = _drone_outline_3d(pts[idx], size=UAV_SQUARE_HALF_SIZE)
            uav_lines[idx].set_data(drone[:, 0], drone[:, 1])
            uav_lines[idx].set_3d_properties(drone[:, 2])
