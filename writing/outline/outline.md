...
- 将实际单轴动力学写成
  \[
  \dot x_{1,i\sigma}=x_{2,i\sigma},\qquad
  \dot x_{2,i\sigma}=F_{i\sigma}(t)+b\,u_{i\sigma},
  \]
  其中
  \[
...
- 引入扩张状态
  \[
  x_{3,i\sigma}=F_{i\sigma}(t),\qquad \dot x_{3,i\sigma}=G_{i\sigma}(t),
  \]
  并据此建立扩张状态空间模型。
- 给出三阶 LESO：
  \[
  ...
  \]
  \[
  \dot z_{2,i\sigma}=z_{3,i\sigma}+b_0u_{i\sigma}-\beta_{02}e_{i\sigma},
  \]
  \[
...
- 存在补偿残差
  \[
  \varepsilon_i^{rope}:=\frac{F_i^{rope}}{m_i}-\hat d_i.
  \]
- 说明：
  - 若 \(\varepsilon_i^{rope}\equiv 0\)，则实际系统退化为名义系统；
  - 若 \(\varepsilon_i^{rope}\) 小且有界，则得到局部 ISS / 一致最终有界口径；
  - 若 \(\varepsilon_i^{rope}(t)\to 0\)，则得到局部渐近收敛口径。
